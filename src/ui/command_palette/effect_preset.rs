//! Effect preset palette — browse and apply/insert effect presets.
...
/// Draw the effect preset palette using the shared fuzzy palette widget.
pub(super) fn draw_effect_preset_palette(
    ctx: &egui::Context,
    state: &mut ResMut<CommandPaletteState>,
    library: &Res<EffectLibrary>,
    selected_effect: Option<Entity>,
            ...
            if items[index].is_new_preset {
                ...
                commands.queue(move |world: &mut World| {
                    ...
                    if let Some(entity) = selected {
                        // Apply to selected effect entity
                ...
                if let Some(marker) = library.effects.get(&preset_name) {
                    ...
                    commands.queue(move |world: &mut World| {
                        if let Some(entity) = selected {
                            // Apply preset to selected effect entity
                            if let Ok(mut e) = world.get_entity_mut(entity) {
                                e.insert(marker);
                            }
                        } else {
...
fn new_preset_name(query: &str, library: &EffectLibrary) -> String {
