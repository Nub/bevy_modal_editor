...
| 3.9 | **Asset browser as file browser** — recursive fuzzy over assets/, directory grouping, save-as-new-file flow, 7 operation flavors | PARTIAL: palette engine + `:e`/`:w` cover the shape; the *file-system browse + save-as* flow needs an explicit M4 line (barrel workflow needs GLB browsing anyway) |
