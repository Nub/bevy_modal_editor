...
| C8 | Hot code reload (dev): a gameplay-system change rebuilds and hot-swaps without restarting the editor session — scene, selection, and history survive the swap. SPIKE FIRST (`spikes/hot-reload`): the approach (dexterous_developer / dylib / subsecond) is validated or the fallback (fast-relaunch: auto-save + auto-restore session state around a restart) is invoked with a ledger note | spike → gate |
