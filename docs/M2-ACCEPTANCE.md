...
| B11 | ~~Macros ride actions~~ **Deferred by owner decision (2026-08-02)**: action-stream recording can't capture mouse gestures (placement clicks, drags), so a replay re-enters modes without reproducing the edits — misleading in a mouse-heavy 3D workflow. Honest macros need gestures-as-data (parameterized actions carrying their spatial inputs) — redesign before reintroduction. The coalescing hook (`MergeFrameEntries`) stays in `editor_core` for scripted batch invocations. | deferred |
