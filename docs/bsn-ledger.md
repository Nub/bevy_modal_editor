...
| 9 | (Toolchain, not BSN) **No viable Rust hot code reload for Bevy 0.19** (spike: `spikes/hot-reload` — dexterous_developer lags Bevy releases, subsecond/dioxus hot-patching unproven past 0.16-era, dylib swap is UB-by-design without a stable ABI) | Fast-relaunch fallback (M3-C8): `editor.reload` saves scene + session sidecar, restarts the rebuilt binary, restores selection/camera/editor-state on boot — pairs with `cargo watch` for the tweak→rebuild→same-scene loop. Undo history is process-local and not preserved | Re-evaluate every phase-boundary Bevy upgrade; adopt dexterous/subsecond when one works on our exact pin | Open |
